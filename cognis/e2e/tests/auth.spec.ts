...
test.describe("Cognis Hire — authentication", () => {
  ...
  test("happy path: e2e user signs in through the Clerk UI with the test OTP", async ({ page }) => {
    ...
    await expect(useAnother.or(page.getByText(/check your email/i)).first()).toBeVisible();
